...
func CreatePoll(c *gin.Context) {
	...
	// 5. Build and insert poll document
	poll := models.Poll{
		ID:          bson.NewObjectID(),
		Title:       input.Title,
	...
	// 6. Seed Redis with zero counts so the SSE snapshot works immediately
...
func GetPoll(c *gin.Context) {
	...
	c.JSON(http.StatusOK, models.PollResult{Poll: poll, Votes: votes})
}
...
// DeletePoll handles DELETE /api/polls/:id  (protected — creator only)
// Removes the poll from MongoDB and cleans up Redis vote data.
func DeletePoll(c *gin.Context) {
	...
	// Only delete if BOTH _id AND creator_id match — prevents deleting another user's poll
	filter := bson.M{"_id": objID, "creator_id": creatorObjID}
	result, err := db.GetCollection("polls").DeleteOne(ctx, filter)
	if err != nil {
	...
	// Clean up Redis keys (non-fatal if they don't exist)
	db.RedisClient.Del(ctx, "poll:"+pollID+":votes", "poll:"+pollID+":voters")
